...
from .diffkd_modules import DiffusionModel, NoiseAdapter, AutoEncoder, DDIMPipeline, Bottleneck
...
class DiffKD(nn.Module):
    def __init__(
        ...
        self.model = DiffusionModel(channels_in=teacher_channels*2, kernel_size=kernel_size)
        ...
        self.process_student_block = Bottleneck(teacher_channels, teacher_channels)
        self.pipeline = DDIMPipeline(self.model, self.scheduler, self.process_student_block, self.noise_adapter)
    ...
    def forward(self, student_feat, teacher_feat):
        ...
        ddim_loss = self.ddim_loss(teacher_feat, refined_feat)


        ###ddim_loss-->diffusion model loss
        ###rec_loss-->autoencoder loss
        return refined_feat, teacher_feat, ddim_loss, rec_loss

    def ddim_loss(self, gt_feat, rf_st_feat):
        ...
        noisy_images = torch.cat((noisy_images, rf_st_feat), dim=1)
