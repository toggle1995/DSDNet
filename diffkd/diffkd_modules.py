    ...
    def __init__(self, channels_in, kernel_size=3):
        ...
        if kernel_size == 3:
            ...
            self.pred = nn.Sequential(
                ...
                nn.BatchNorm2d(channels_in * 4),
                nn.ReLU(inplace=True),
                nn.Conv2d(channels_in * 4, channels_in // 2, 1)
            )
...
class DDIMPipeline:
    ...
    def __init__(self, model, scheduler, process_feat, noise_adapter=None, solver='ddim'):
        super().__init__()
        self.model = model
    ...
    def __call__(
        ...
        if self.noise_adapter is not None:
            ...
            # # pdb.set_trace()
            # process_feat = self.process_feat(feat)
            # image = self.scheduler.add_noise_diff2(process_feat, noise, timesteps)
            image = feat
        else:
            image = feat
        ...
        image_output = torch.randn(image.shape, device=image.device) #.to(gt_feat.device)
        
        

        for t in self.scheduler.timesteps[len(self.scheduler.timesteps)//2:]:
            image_concat = torch.cat((image_output, image), dim=1)
            noise_pred = self.model(image_concat, t.to(device))

            # 2. predict previous mean of image x_t-1 and add variance depending on eta
            # eta corresponds to η in paper and should be between [0, 1]
            # do x_t -> x_t-1
            image_output = self.scheduler.step(
                noise_pred, t, image_output, eta=eta, use_clipped_model_output=True, generator=generator
            )['prev_sample'] 
