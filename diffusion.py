...
from function.functions import KLDivergence, DIST
...
class KDLoss(nn.Module):
    def __init__(
        ...
        self.diff = DiffKD(self.student_channel, self.teacher_channel, kernel_size=1, use_ae=False, ae_channels=45)
        self.kd_loss = KLDivergence(tau=1)
        self.diff.cuda()
    ...
    def forward(self, f_tea, f_stu):
        ...
        # kd_loss = self.kd_loss(f_stu, f_tea)



        student_out, teacher_out, diff_loss, ae_loss = self.diff(self._reshape_BCHW(f_stu), self._reshape_BCHW(f_tea))
        if ae_loss is not None:
            kd_loss_all += diff_loss + ae_loss
        else:
            kd_loss_all += diff_loss
        
        # kd_loss_all += kd_loss
        
        return kd_loss_all, student_out, teacher_out
        ...
        #     # compute kd loss
        #     ##### kd_loss --> diffusion knowledge distillation loss
